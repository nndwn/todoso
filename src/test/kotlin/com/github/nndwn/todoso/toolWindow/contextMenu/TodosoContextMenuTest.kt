...
class TodosoContextMenuTest : BasePlatformTestCase() {
  ...
  fun testBuildMenuStructure() {
    ...
    // Mock view actions
    val view =
      object : TodosoActionHandler.TodoViewActions {
    ...
    assertTrue(
      "Editor actions should be present with selection",
      elements.any { it is TodosoMenuElement.Action && it.text.contains("Edit") },
    )

    val statusSubMenu = elements.filterIsInstance<TodosoMenuElement.SubMenu>().find { it.text.contains("Status") }
